    ...
    for id in lista_id:
        ...
        for key in dictionary.keys():
            for riga_splittata in dictionary[key]:
                if riga_splittata[0]==id:
                    if riga_splittata[2]=='0' or riga_splittata[2]=='NA' or riga_splittata[2]=='N/A':
                        name = riga_splittata[1]
                    else:
                        name = riga_splittata[1]
        ...
        listarighe_nuovofile.append(id+';'+name+';'+str(totcount)+';'+str(totarea)+';'+str(avrgdensity)+';'+str(densities_stdev)+';'+str(countfettine)+';'+str(countideale)+'\n')
    return listarighe_nuovofile


lista_csv = get_list_of_files_in_directory('RefAtlas_with_density')
dictionary = create_dict_Key_csv_Value_lista_annidata_righe(lista_csv)
lista_id, dizionario_idVScount = obtain_lista_id_e_dizionario('id_aree_da_considerare.txt', 'count_ideali.txt')
with open(os.getcwd()[-3:]+'_unito.csv', 'a')as f:
