...
def main():
    ...
    parser.add_argument('-i', '--input', help="input csv file", default='C:/Users/tomgo/Desktop/PLS_python/PLS/MC-HC/PLS_MC_HC.csv')
    ...
    parser.add_argument('-n', '--n_permutations', help="number of permutations for the permutation test", type=int,
                        default=100000)
...
def plot_contrast_histogram(data, title, row, output_file):
    ...
    bars = ax.bar(data.columns, data.iloc[row],
                  color=[group_colors['HC'] if col == 'group_HC' else group_colors['MC'] for col in data.columns],
    ...
    ax.legend(bars[:2], ['HC', 'MC'], loc='upper right', bbox_to_anchor=(1.5, 1.5))
    plt.tight_layout()
