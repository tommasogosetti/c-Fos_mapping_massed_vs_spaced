...
def permutation_test(xm, ym, n_permutations=1000):
    """
...
def main():
    ...
    parser.add_argument('-i', '--input', help="input csv file",
                        default='C:/Users/tomgo/Desktop/PLS_python/PLS/SS-SC/PLS_SS_SC.csv')
    ...
    logger.info('Performing ANOVA on contrasts...')
    contrasts = upd[['group_SS', 'group_SC']]

    # Esegui ANOVA
    f_value, p_value_anova = stats.f_oneway(contrasts['group_SS'], contrasts['group_SC'])
    logger.info(f"ANOVA F-value: {f_value}, p-value: {p_value_anova}")

    # Se ANOVA è significativa, esegui test post-hoc
    if p_value_anova < alpha:
        logger.info("ANOVA significativa, eseguendo test post-hoc di Tukey...")
        melted = contrasts.melt(var_name='Group', value_name='Contrast')
        tukey_result = pairwise_tukeyhsd(melted['Contrast'], melted['Group'])
        logger.info(f"Risultati del test di Tukey:\n{tukey_result}")
    else:
        logger.info("ANOVA non significativa, nessun test post-hoc necessario.")
...
def plot_contrast_histogram(data, title, row, output_file):
    ...
    bars = ax.bar(data.columns, data.iloc[row],
                  color=[group_colors['MC'] if col == 'group_MC' else group_colors['SC'] for col in data.columns],
    ...
    ax.legend(bars[:2], ['MC', 'SC'], loc='upper right', bbox_to_anchor=(1.5, 1.5))
    plt.tight_layout()
